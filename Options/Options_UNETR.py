...
class Options_UNETR_FullPICAI(BaseOptions):
    """Training options for UNETR on Dataset141_FullPICAI."""
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--task_name', type=str, default='SegGland_UNETR_FullPICAI', help='the current task name')
        parser.add_argument('--dice_weight', type=float, default=0.5, help='weight for Dice loss')
        parser.add_argument('--focal_weight', type=float, default=0.5, help='weight for Focal loss')
        parser.add_argument('--batch_size', type=int, default=16, help='input train batch size')
        parser.add_argument('--resume', type=str, default=None, help='checkpoint path for resume training')
        parser.set_defaults(lr=1e-4, gpu_ids='2', epoch=200, model_save_fre=20)
        self.isTrain = True
        return parser
