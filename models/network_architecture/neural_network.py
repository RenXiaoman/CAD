...
try:
    ...
    def pad_nd_image(image, new_shape=None, mode="constant", kwargs=None, return_slicer=False, shape_must_be_divisible_by=None):
        ...
        if shape_must_be_divisible_by is not None:
            divisor = np.array(shape_must_be_divisible_by)
            new_shape = np.maximum(new_shape, old_shape)
            new_shape = new_shape + (divisor - new_shape % divisor) % divisor
        else:
            new_shape = np.maximum(new_shape, old_shape)

        pad_shape = new_shape - old_shape
        pad_before = pad_shape // 2
        pad_after = pad_shape - pad_before
        pad_width = [(0, 0)] * (image.ndim - len(new_shape))
        pad_width += [(int(before), int(after)) for before, after in zip(pad_before, pad_after)]
        padded = np.pad(image, pad_width, mode=mode, **kwargs)

        slicer = tuple(
            [slice(None)] * (image.ndim - len(new_shape))
            + [slice(int(before), int(before + size)) for before, size in zip(pad_before, old_shape)]
        )
        return (padded, slicer) if return_slicer else padded
...
try:
    ...
    def gaussian_filter(input, sigma, order=0, mode="reflect", cval=0.0, truncate=4.0):
        return input
