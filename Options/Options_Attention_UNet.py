...
class Options_A_UNet_PICAI(BaseOptions):
    """This class includes training options.
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--task_name', type=str, default='SegTumor_Attention_UNet_PI-CAI_NoAugment', help='the current task name')
        ...
        parser.add_argument('--resume', nargs='?', const=False, default=True, help='resume training from checkpoint. True uses checkpoints_dir/task_name/model_latest.pth')
        parser.set_defaults(lr=1e-4, gpu_ids='1', epoch=200)
        self.isTrain = True
        return parser
