...
BASE = Path('/media/lib/D/renxiaoman/PyCharmProjects/MoSID/dataset/ChengdaOnlyCSPca')
NNUNET_SRC_DIR = BASE / "nnUNet_train_origin"
NNUNET_DEST_DIR = BASE / "nnUNet_train"
...
def process_case(patient_id: str):
    ...
    # 构建源文件路径
    src_imagesTs_dir = NNUNET_SRC_DIR / "imagesTs"
    src_labelsTs_dir = NNUNET_SRC_DIR / "labelsTs"
    ...
    try:
        ...
        # 构建目标文件路径
        dest_imagesTs_dir = NNUNET_DEST_DIR / "imagesTs"
        dest_labelsTs_dir = NNUNET_DEST_DIR / "labelsTs"
...
def get_patient_ids():
    ...
    labelsTs_dir = NNUNET_SRC_DIR / "labelsTs"
    if not labelsTs_dir.exists():
        print("[ERROR] labelsTs directory not found!")
        return []
