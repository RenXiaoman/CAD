...
class VSNet(nn.Module):
    ...
    def forward(self,x):  # [4, 1, 96, 96, 96]
        x1  = self.resuetencoder1(x)  # [4, 12, 96, 96, 96]

        x2 = self.resuetencoder2(x1)  # [4, 24, 96, 96, 96]
        x2 = self.pool2(x2)  # [4, 24, 48, 48, 48]

        x1 = self.gate2(x1,x2)  # [4, 12, 96, 96, 96]
        
        x3 = self.resuetencoder3(x2)  # [4, 48, 48, 48, 48]
        x3 = self.pool3(x3)  # [4, 48, 24, 24, 24]
        
        x2 = self.gate3(x2,x3)  # [4, 24, 48, 48, 48]
        
        x4 = self.resuetencoder4(x3)  # [4, 96, 24, 24, 24]
        x4 = self.pool4(x4)  # [4, 96, 12, 12, 12]
        
        x3 = self.gate4(x3,x4)  # [4, 48, 24, 24, 24]
        
        x5 = self.swintransformer(x4.contiguous())  # [4, 192, 6, 6, 6]
...
if __name__ == "__main__":
    ...
    model = VSNet(img_size=image_size, in_channels=1, out_channels=2, training=False)
