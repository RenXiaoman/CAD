...
# Local imports
from Options.Options_AWUNet import Options_AWUNet_AHCDU
...
def main(options_class=Options_AWUNet_AHCDU, train_augmentation=True):
    # Parse options
    ...
    # Initialize model
    model = build_aw_unet(num_classes=2, num_res_units=opt.num_res_units).to(device)
    ...
    # Define loss function from AWA_BoundaryDoULoss.
    boundary_dou_loss = build_boundary_dou_loss(num_classes=2).to(device)
    ...
    for epoch in range(start_epoch, opt.epoch):
        ...
        for batch_idx, batch_data in enumerate(tqdm(train_dataloader, desc=f'Epoch {epoch+1}/{opt.epoch} [Train]')):
            ...
            # Calculate loss
            loss = boundary_dou_loss(outputs, labels)
