...
class Options_ResGNet_AHCDU(BaseOptions):
    """Training options for ResGNet on AHCDU."""
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--task_name', type=str, default='SegGland_ResGNet_AHCDU', help='the current task name')
        ...
        parser.set_defaults(lr=1e-4, gpu_ids='2', epoch=50)
        self.isTrain = True
        return parser


class Options_ResGNet_PICAI(BaseOptions):
    """Training options for ResGNet on PICAI."""
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--task_name', type=str, default='SegGland_ResGNet_PICAI_NoAug', help='the current task name')

        parser.add_argument('--dice_weight', type=float, default=0.5, help='weight for Dice loss')
        parser.add_argument('--focal_weight', type=float, default=0.5, help='weight for Focal loss')
        parser.add_argument('--batch_size', type=int, default=1, help='input train batch size')
        parser.add_argument('--resume', type=bool, default=True, help='resume training from checkpoint')
        parser.set_defaults(lr=1e-4, gpu_ids='1', epoch=50)
        self.isTrain = True
        return parser


class Options_ResGNet_FullPICAI(BaseOptions):
    """Training options for ResGNet on the full PI-CAI split (Dataset141)."""
    ...
    def initialize(self, parser):
        ...
        parser.add_argument(
            '--task_name',
            type=str,
            default='SegGland_ResGNet_FullPICAI',
        ...
        parser.set_defaults(lr=1e-4, gpu_ids='3', epoch=50)
        self.isTrain = True
        return parser
